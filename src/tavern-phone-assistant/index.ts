...
function sendToTavernInput(text: string): void {
  try {
    ...
    const ta = doc.querySelector<HTMLTextAreaElement>(
      '#send_textarea, textarea.send_text, .send_form textarea'
    );
    if (!ta) {
    ...
    // Set value and trigger input event so tavern detects it
    ...
    const sendBtn = doc.querySelector<HTMLElement>(
      '#send_but, .send_button, button[title="Send"], .mes_btn_send'
    );
    if (sendBtn) {
      sendBtn.click();
...
function phoneToTavernBridge(content: string): void {
  ...
  // Try API first, fallback to input injection
  const sent = sendToTavernAPI(content);
  if (!sent) {
    sendToTavernInput(content);
  }

  // Optionally trigger AI generation
...
function togglePanel(show?: boolean): void {
  ...
  if (panelVisible) {
    renderMessages();
    // Scroll to bottom
    const msgsEl = document.getElementById('pa-messages');
...
function toggleSettings(show?: boolean): void {
  const settingsEl = document.getElementById('pa-settings');
...
function renderSettings(): void {
  ...
  const tagInput = document.getElementById('pa-tag-input') as HTMLInputElement;
  if (tagInput) tagInput.value = s.outTag;

  const contactInput = document.getElementById('pa-contact-input') as HTMLInputElement;
...
function bindEvents(): void {
  ...
  // Close panel
  document.getElementById('pa-close-btn')?.addEventListener('click', () => togglePanel(false));

  // Settings toggle
  document.getElementById('pa-settings-btn')?.addEventListener('click', () => {
    renderSettings();
    toggleSettings(true);
  });

  // Settings back
  document.getElementById('pa-settings-back')?.addEventListener('click', () => {
  ...
  // Send message
  const sendBtn = document.getElementById('pa-input-send');
  const inputField = document.getElementById('pa-input-field') as HTMLTextAreaElement;
  ...
  observer.observe(document.body, { attributeFilter: ['class'], subtree: true });
}
...
function init(): void {
  // If loaded in iframe with no parent access, show fallback
  try {
    if (window.top === window) {
      // Running standalone — create full page version
      ...
      wrapper.id = 'pa-standalone-wrapper';
